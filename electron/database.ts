...
export function getAllGoals(): Goal[] {
  ...
  return rows.map(row => {
    ...
    const linkedCategories = (catStmt.all(row.id) as any[]).map(r => r.category);
    
    // Get linked bills
    const billStmt = db.prepare('SELECT bill_name FROM goal_linked_bills WHERE goal_id = ?');
    const linkedBillNames = (billStmt.all(row.id) as any[]).map(r => r.bill_name);
...
export function getSettings(): Settings {
  ...
  const settingsRows = settingsStmt.all('currency', 'uiMode') as any[];
  
  const settingsMap = new Map(settingsRows.map(r => [r.key, r.value]));
  
  // Get categories
  const categoriesStmt = db.prepare('SELECT id, name, limit_amount FROM categories');
  const categories = (categoriesStmt.all() as any[]).map(row => ({
    id: row.id,
    name: row.name,
  ...
  return {
    ...
    uiMode: (settingsMap.get('uiMode') as any) || 'professional',
...
export function addCategory(category: { id?: string; name: string; limit: number }): void {
  const db = getDatabase();
